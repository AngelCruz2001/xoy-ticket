...
export const Table = () => {
    ...
    useEffect(() => {
        if (tickets) {
            tickets?.map((ticket, index) => {
                QRCode.toDataURL(ticket.Transaccion, (_, url) => (setTicketsData(tickets?.map((ticket, index) => ({ ...ticket, qrCode: url })))));
            });
        }
    }, [tickets]);
    ...
    return (<Card>
      ...
      <div className='list'>
        ...
        {ticketsData?.map(ticket => (<Row key={ticket.transaccion}>
              <button className='seeButton' onClick={() => handleActiveTicket(ticket)}>
        ...
        {activeTicket !== null &&
            ...
            {activeTicket.boletos.map((seat, i) => (<Row key={seat.id} extra={true}>
                <Cell text={seat.Area}/>
                <Cell text={seat.Fila}/>
